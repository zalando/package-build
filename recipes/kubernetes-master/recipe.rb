...
class KubernetesMaster < FPM::Cookery::Recipe
  ...
  def install
    ...
    bin.install builddir("kubernetes-1.0.3/_output/local/bin/linux/amd64/kube-controller-manager")
    bin.install builddir("kubernetes-1.0.3/_output/local/bin/linux/amd64/kube-apiserver")
    bin.install builddir("kubernetes-1.0.3/_output/local/bin/linux/amd64/kube-scheduler")
  end
end
